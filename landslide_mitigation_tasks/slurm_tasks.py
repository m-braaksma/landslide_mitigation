...
#                 # Use union of all adm_ids from all sources
#                 all_adm_ids = set()
#                 if sed_stats:
#                     all_adm_ids.update(sed_stats.keys())
#                 if pop_stats:
...
#                     # Add all sed_export stats
#                     if sed_stats:
#                         for stat_key, stat_value in sed_stats.get(adm_id, {}).items():
#                             record[f'sed_export_{stat_key}'] = stat_value
                    
#                     # Add all worldpop stats
#                     if pop_stats:
#                         for stat_key, stat_value in pop_stats.get(adm_id, {}).items():
#                             record[f'worldpop_{stat_key}'] = stat_value

#                     # Add all cf_sed_export stats
#                     if cf_sed_stats:
#                         for stat_key, stat_value in cf_sed_stats.get(adm_id, {}).items():
#                             record[f'cf_sed_export_{stat_key}'] = stat_value
                
#                     # Add all era5 stats
#                     if era5_stats:
#                         for stat_key, stat_value in era5_stats.get(adm_id, {}).items():
#                             record[f'era5_stats_{stat_key}'] = stat_value
...
# def combine_zonal_stats_task(p):
#     """Combine all zonal statistics results."""
#     if p.run_this:
#         # Define expected output file 
#         # Save to repo so that the analysis can be picked up locally here
#         code_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
#         # Check if output already exists and skip if not forcing
#         if os.path.exists(output_file) and not p.force_run:
#             hb.log(f"Output file already exists: {output_file}")
#             hb.log("Skipping combine zonal statistics (use force_run=True to override)")
#             return p
...
#         try:
#             # Run the combine function directly
#             result_path = combine_zonal_stats(p.proj_data_dir, p.cur_dir, p.years)
#             hb.log(f"Successfully combined zonal statistics: {result_path}")
...
# def combine_zonal_stats(proj_data_dir, output_dir, years):
#     """Combine all zonal statistics results into a single DataFrame."""
#     import pandas as pd
#     import pickle
#     import sys
...
#     hb.log("Starting to combine zonal statistics results...")
#     hb.log(f"Project data dir: {proj_data_dir}")
#     hb.log(f"Output dir: {output_dir}")
#     hb.log(f"Years: {years}")
...
#             # Define S3 paths
#             sed_export_s3_path = proj_data_dir + f"zonal_stats/sed_export_{year}_zonal_stats.pkl"
#             cf_sed_export_s3_path = proj_data_dir + f"zonal_stats/sed_export_cf_{year}_zonal_stats.pkl"
#             worldpop_s3_path = proj_data_dir + f"zonal_stats/worldpop_{year}_zonal_stats.pkl"
#             era5_s3_path = proj_data_dir + f"zonal_stats/era5_precip_{year}_zonal_stats.pkl"
...
#         # Use union of all adm_ids from both sources
#         all_adm_ids = set()
#         if sed_stats:
#             all_adm_ids.update(sed_stats.keys())
#         if pop_stats:
...
#     # Save directly to the specified output directory
#     output_local_path = os.path.join(output_dir, 'zonal_stats_adm2.csv')
#     os.makedirs(output_dir, exist_ok=True)  # Ensure directory exists
#     df.to_csv(output_local_path, index=False)
    
#     hb.log(f"Combined zonal statistics saved to: {output_local_path}")
#     hb.log(f"DataFrame shape: {df.shape}")
#     hb.log(f"Columns: {list(df.columns)}")
